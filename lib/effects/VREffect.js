...
THREE.VREffect = function ( renderer, onError ) {
	...
	this.autoSubmitFrame = false;

	// render

	var perspectiveCameraL = new THREE.PerspectiveCamera();
	perspectiveCameraL.layers.enable( 1 );

	var perspectiveCameraR = new THREE.PerspectiveCamera();
	perspectiveCameraR.layers.enable( 2 );

  this.isVRActive = function() {
    return vrDisplay && scope.isPresenting;
  }
	...
	this.render = function ( scene, camera, renderTarget, forceClear ) {
      ...
      let baseCamera = camera;
      let cameraL = perspectiveCameraL;
      let cameraR = perspectiveCameraR;

      if ( camera.parent === null ) {
        camera.updateMatrixWorld();
      } else if (camera.parent.name === 'camera-group') {
        baseCamera = camera.parent;
      }

			baseCamera.matrixWorld.decompose( cameraL.position, cameraL.quaternion, cameraL.scale );
