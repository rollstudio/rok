...
class RuneMaster {
  ...
  _show(time) {
    ...
    // show rune grid
    this._runeGrid.show(this._currentRune, this._duration, this._hide);
  ...
  activate(time, rune, duration, callback) {
    if (!this._isActive) {
      this._isActive = true;
      this._startTime = time;
      this._currentRune = rune;
      this._duration = duration;

      console.log('[RuneMaster] activated for', rune);

      this._doneHandler = callback;
      this._show(time);
    } else {
      callback();
    }
