...
export default class RayController extends EventEmitter {
  ...
  getInteractionMode() {
    ...
    if (gamepad) {
      ...
      if (pose) {
        ...
        console.warn('Detected gamepad has no pose attribute. Check for "#enable-gamepad-extensions".');
      }
    } else {
  ...
  onMouseDown_(e) {
    this.app.emitter.emit('pressed',true);
  ...
  onMouseUp_(e) {
    this.app.emitter.emit('pressed',false);
